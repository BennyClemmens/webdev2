...
for (const nl of nl_en_dict.keys()) {
  console.log(nl);
}
...
function gelijk(woordenboek) {
  return [...woordenboek].reduce((pv, [k, v]) => (k === v ? pv + 1 : pv), 0);
}
console.log(gelijk(nl_en_dict));
console.log(gelijk(en_fr_dict));
...
krishna.forEach((el) => setje.add(el));
for (const s of setje) {
  console.log(s);
}
...
const oneliner = [...new Set(krishna)];
oneliner.forEach((o) => console.log(o));
