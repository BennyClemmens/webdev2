...
population.clear();
console.log(
  `The map has been cleared. It contains ${population.size} entries...`
);
...
let message = "";
console.log(
  `Use entries-method to get an iterable with all the entries from the map`
);
...
console.log(
  `Use keys-method to get an iterable with all the keys from the map`
);
...
console.log(
  `Use values-method to get an iterable with all the values from the map`
);
...
console.log(
  `Use forEach-method to iterate over a map with a callback function`
);
message = "Countries with less than 5000000 people:\n";
population.forEach((value, key) => {
  if (value < 5000000) message += `${key} with ${value} people\n`;
});
console.log(message);
...
population.set(bel, 11589623).set(zim, 14862924).set(col, 97338579);
...
console.log(
  `Hope you have a good understanding of strict equality, look at the console ;-)`
);
